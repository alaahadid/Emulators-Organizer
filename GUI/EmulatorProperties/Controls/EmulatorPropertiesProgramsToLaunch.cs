...
{
    public partial class EmulatorPropertiesProgramsToLaunch : IEmulatorPropertiesControl
    {
        public EmulatorPropertiesProgramsToLaunch()
        {
            InitializeComponent();
        }
        ...
        public override string Description
        {
            get
            {
                return ls["EmulatorPropertiesDescription_ProgramsToLaunch"];
            }
        }
        public override void LoadSettings()
        {
            Emulator emulator = profileManager.Profile.Emulators[emulatorID];
            // programs before
            if (emulator.ProgramsToLaucnhBefore != null)
            {
                foreach (ProgramProperties prog in emulator.ProgramsToLaucnhBefore)
                {
            ...
            // programs after
            if (emulator.ProgramsToLaucnhAfter != null)
            {
                foreach (ProgramProperties prog in emulator.ProgramsToLaucnhAfter)
                {
        ...
        {
            //programs before
            profileManager.Profile.Emulators[emulatorID].ProgramsToLaucnhBefore = new List<ProgramProperties>();
            foreach (ListViewItem item in listView_programsBefore.Items)
            {
                ...
                profileManager.Profile.Emulators[emulatorID].ProgramsToLaucnhBefore.Add(prog);
            }
            //programs after
            profileManager.Profile.Emulators[emulatorID].ProgramsToLaucnhAfter = new List<ProgramProperties>();
            foreach (ListViewItem item in listView_lauchAfter.Items)
            {
                ...
                prog.BatMode = tagged.BatMode;
                prog.BatScript = tagged.BatScript;
                profileManager.Profile.Emulators[emulatorID].ProgramsToLaucnhAfter.Add(prog);
            }
        }
        // Add program before
