...
{
    public partial class Form_AddRomsFolderScan : Form
    {
        public Form_AddRomsFolderScan(string consoleID)
        {
            InitializeComponent();
            // Refresh selection
            console = profileManager.Profile.Consoles[consoleID];
            if (console.Memory_RomFolders == null)
                console.Memory_RomFolders = new List<string>();
            foreach (string folder in console.Memory_RomFolders)
            {
        ...
        {
            // Add listener
            logPath = ".\\Logs\\" + DateTime.Now.ToLocalTime().ToString() + "-add roms (folder scan).txt";
            ...
            Trace.WriteLine("Add roms to console '" + console.Name + "' started at " +
                DateTime.Now.ToLocalTime(), "Add roms (folders scan)");
            Trace.WriteLine("Folders count= " + folders.Count, "Add roms (folders scan)");
            List<string> files = new List<string>();
            foreach (string folder in folders)
            {
                // getting files for folder
                Trace.WriteLine("Getting files from folder '" + folder + "'", "Add roms (folders scan)");
                files.AddRange(Directory.GetFiles(folder, "*",
                    _includeSubfolders ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly));
            }
            Trace.WriteLine("Total files found= " + files.Count + " file(s)", "Add roms (folders scan)");
            bool needToSerachForMatchedRom = true;
            // clear old collection if requested by user
            if (_clearAllRoms)
            {
                Trace.WriteLine("Clearing roms that belong to console '" + console.Name + "' by the request of the user.", "Add roms (folders scan)");
                profileManager.Profile.Roms.RemoveThatBelongToConsole(console.ID);
                Trace.WriteLine("Roms removed from console '" + console.Name + "'", "Add roms (folders scan)");
            }
            ...
            // adding roms
            Trace.WriteLine("Adding roms ....", "Add roms (folders scan)");
            int i = 0; added_roms_count = 0;
            foreach (string file in files)
            {
                if (_useAIPath)
                {
                    ...
                    {
                        Trace.WriteLine("File is not an archive ! adding file normally.", "Add roms (folders scan)");
            ...
            Trace.WriteLine("Add roms (folder scan) finished at " + DateTime.Now.ToLocalTime(), "Add roms (folders scan)");
            listner.Flush();
        ...
        private void CloseAfterFinish1()
        {
            finished = true;
            timer1.Stop();
            listView1.Enabled = button1.Enabled = button2.Enabled = checkBox_clearRomsCollectionFirst.Enabled = checkBox_includeSubfolders.Enabled =
              radioButton_ignore_rom.Enabled = radioButton_replace_rom.Enabled = button_start.Enabled = true;
        ...
        private void SaveMemoryFolders()
        {
            if (!this.InvokeRequired)
                SaveMemoryFolders1();
            else
                this.Invoke(new Action(SaveMemoryFolders1));
        }
        private void SaveMemoryFolders1()
        {
            console.Memory_RomFolders = new List<string>();
            foreach (ListViewItem item in listView1.Items)
            {
                console.Memory_RomFolders.Add(item.Text);
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            folder.Description = ls["Description_BrowseForRomsFolder"];
            // Set selected directory
            if (listView1.Items.Count > 0)
                folder.SelectedPath = listView1.Items[listView1.Items.Count - 1].Text;
            else if (console.Memory_RomFolders != null)
            {
                if (console.Memory_RomFolders.Count > 0)
                {
                    folder.SelectedPath = console.Memory_RomFolders[console.Memory_RomFolders.Count - 1];
                }
            }
            if (folder.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
            {
                foreach (ListViewItem item in listView1.Items)
                {
                    if (item.Text.ToLower() == folder.SelectedPath)
                        return;
                }
                ListViewItem newItem = new ListViewItem();
                newItem.Text = folder.SelectedPath;
                newItem.ImageIndex = 0;
                listView1.Items.Add(newItem);
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (listView1.Items.Count == 0)
            {
                ManagedMessageBox.ShowErrorMessage(ls["Message_PleaseAddFoldersFirst"], ls["MessageCaption_AddRoms"]);
                return;
            }
            // disable things
            ServicesManager.OnDisableWindowListner();
            listView1.Enabled = button1.Enabled = button2.Enabled = checkBox_clearRomsCollectionFirst.Enabled = checkBox_includeSubfolders.Enabled =
                    checkBox_use_ai_path.Enabled = textBox_archive_extensions.Enabled = linkLabel1.Enabled = checkBox_alwaysChooseChild.Enabled =
                radioButton_ignore_rom.Enabled = radioButton_replace_rom.Enabled = button_start.Enabled = checkBox_useParent.Enabled = false;
            // get options
            _includeSubfolders = checkBox_includeSubfolders.Checked;
            _clearAllRoms = checkBox_clearRomsCollectionFirst.Checked;
            _replaceRom = radioButton_replace_rom.Checked;
        ...
        private void Form_AddRomsFolderScan_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!finished)
            {
                if (mainThread != null)
