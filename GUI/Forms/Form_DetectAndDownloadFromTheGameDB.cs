...
{
    public partial class Form_DetectAndDownloadFromTheGamesDB : Form
    {
        public Form_DetectAndDownloadFromTheGamesDB(string consoleID)
        {
            InitializeComponent();
      ...
      */
        ...
        private bool Compare_RomPath_EntryName(string romFilePath, string entryFileNames)
        {
            ...
            int matchedNumber = 0;
            // Remove forbidden values   
            string fileOfDatabase = entryFileNames.Replace(":", " ");
            fileOfDatabase = fileOfDatabase.Replace("|", "");
            fileOfDatabase = fileOfDatabase.Replace(@"\", "");
            fileOfDatabase = fileOfDatabase.Replace("/", "");
            fileOfDatabase = fileOfDatabase.Replace("*", "");
        ...
        private void AddTabConentFilesToRomTEXT(string tabName, string folder, string file_content, Rom rom, bool clearOldList)
        {
            foreach (InformationContainer ic in profileManager.Profile.Consoles[consoleID].InformationContainers)
            {
                if (ic.DisplayName.ToLower() == tabName.ToLower())
       ...
       /* private void ApplyRom(Rom rom, Game game)
        {
            ...
            if (_db_add_overview_as_tab)
            {
                if (_db_create_new_tab_for_overview)
                {
                    ...
                    AddTabConentFilesToRomTEXT("Overview", _db_overview_folder, game.Overview, rom, _db_overview_ic_clearlist);
                }
                else
                {
                    // Assign to a tab !
                    ...
                    // Download the tabs for it !
                    AddTabConentFilesToRomTEXT(tabName, _db_overview_folder, game.Overview, rom, _db_overview_ic_clearlist);
                }
            }
            #endregion
            #region Banners
