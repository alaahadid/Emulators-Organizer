...
{
    public partial class Form_FindRoms : Form
    {
        public Form_FindRoms()
        {
            InitializeComponent();
            this.Tag = "find";
            RefreshModeCombobox();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                ManagedMessageBox.ShowErrorMessage(ls["Message_PleaseEnterSearchWord"], ls["MessageCaption_FindRoms"]);
                return;
            }
            if (comboBox_searchBy.SelectedIndex < 0)
            {
                ManagedMessageBox.ShowErrorMessage(ls["Message_PleaseSelectSearchBy"], ls["MessageCaption_FindRoms"]);
                return;
            }
            ...
            if (!isDataItem)
            {
                profileManager.Profile.OnRequestSearch(new SearchRequestArgs(textBox_findWhat.Text, mode,
                   textCondition, numberCondition, checkBox_caseSensitive.Checked));
            }
            else
            {
                profileManager.Profile.OnRequestSearch(new SearchRequestArgs(textBox_findWhat.Text, dataItemName,
                   textCondition, numberCondition, checkBox_caseSensitive.Checked));
            }
        }
